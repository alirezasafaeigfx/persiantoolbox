  ...
  return (
    <SiteShell containerClassName="py-10">
      ...
      >
        ...
        <span aria-hidden="true">/</span>
        <Link href="/blog/author" className="hover:text-primary">
          نویسندگان
        </Link>
        <span aria-hidden="true">/</span>
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-3xl font-black text-(--text-primary)">{author.name}</h1>
          <p className="text-sm text-(--text-secondary) mt-1">{author.bio}</p>
          <p className="text-xs text-(--text-muted) mt-2">{posts.length} مقاله</p>
        </div>
      </div>
      ...
      {posts.length > 0 ? (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {posts.map((post) => (
            <Link
              key={post.slug}
              href={`/blog/${post.slug}`}
              className="group rounded-lg border border-(--border-light) bg-(--surface-1) p-5 transition-all hover:border-primary/30 hover:shadow-medium"
            >
              <h2 className="text-base font-bold text-(--text-primary) group-hover:text-primary transition-colors line-clamp-2">
                {post.title}
              </h2>
              <p className="mt-2 text-xs text-(--text-muted) line-clamp-2">{post.description}</p>
        ...
        <p className="text-sm text-(--text-muted)">هنوز مقاله‌ای منتشر نشده است.</p>
      )}
    </SiteShell>
  );
}
