...
- source reconciliation — 2026-08-08: export رسمی Search Console کاربر (`Last 3 months`, Web، تا 2026-08-06) اعداد API را مستقل تأیید کرد: 7d برابر 202 click/2,128 impression/9.49% CTR؛ 28d برابر 604/5,449/11.08%. در 92 روز صفحهٔ آدرس 442 click/1,208 impression/36.59% CTR/position 3.63 داشت. فرصت‌های بعدی ثبت‌شده، بدون شروع آزمایش متداخل: `ocr فارسی` با 370 impression/5.95% CTR/position 7.96 و `محاسبه تاخیر تادیه رایگان` با 368 impression/1.09% CTR/position 11.49.
- draft PR: `https://github.com/alirezasafaei-dev/persiantoolbox/pull/193` با base=`main` ایجاد شد؛ release/deploy همچنان مستقل و انجام‌نشده است.
- CI remediation — 2026-08-08: check `security-audit` در run `31278279117` به‌دلیل 6 high advisory موجود شکست خورد. overrideهای transitive فقط به patch امن ارتقا یافتند (`fast-uri 3.1.5`, `brace-expansion 5.0.9`, `js-yaml 3.15.1`, `nanoid 3.3.17/5.1.16`). فرمان دقیق CI اکنون صفر high و فقط 2 moderate گزارش می‌کند و exit 0 است؛ full Vitest 207/1,671، typecheck، lint، local-first، licensing و build 960-page همگی دوباره exit 0 شدند.
- CI remediation نهایی — 2026-08-08: دو advisory متوسط باقی‌مانده نیز با patch مستقیم `dompurify 3.4.13` و `postcss 8.5.23` رفع شد. `pnpm audit --prod --json` اکنون 0 info/low/moderate/high/critical گزارش می‌کند؛ full Vitest همچنان 207 فایل/1,671 تست با صفر failure، typecheck/lint/licensing exit 0 و production build نهایی با 960 صفحه و standalone artifact exit 0 است. یک install محلی نیمه‌کاره ابتدا فایل‌های package store را ناقص کرده بود؛ با بازسازی همان نسخه‌های قفل‌شده از cache محلی، build بدون تغییر سورس یا lockfile اضافی تأیید شد.
- release — 2026-08-09: PR `#193` در `main` merge شد و پس از رفع مانع فرایند legacy در PR `#195`، release با SHA دقیق `bb53003327e513be21b3bd64d98a975771c09054` در ساعت `2026-08-09T08:15:24Z` روی production فعال شد. workflow `deploy-production` run `31302043931`، ممیزی مستقل active/previous slot، گزارش strict، سلامت دیتابیس، ۱۰ مسیر اجباری، CSS و font همگی سبز بودند.
- release status: `RELEASED`; instrumentation چرخه اکنون روی production فعال است.
- earliest evaluation date: `2026-08-16T08:15:24Z` (`T_release + 7d`) و فقط پس از حداقل 30 رخداد `tool_start` برای `tool_id=address-fa-to-en`؛ قبل از هر دو شرط، نتیجه `inconclusive` است و آزمایش متداخل آغاز نمی‌شود.
