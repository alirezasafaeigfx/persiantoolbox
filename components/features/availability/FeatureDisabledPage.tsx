...
export default function FeatureDisabledPage({ feature }: Props) {
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6 py-6">
      <div className="inline-flex items-center gap-2 rounded-full border border-(--border-light) bg-(--surface-1) px-3 py-1 text-xs font-semibold text-(--text-muted)">
        <IconShield className="h-3.5 w-3.5 text-primary" />
        تجربه آفلاین + حریم خصوصی
      </div>
      <Card className="p-6 md:p-8 space-y-4">
        <div className="flex items-start gap-3">
          <div className="mt-1 rounded-full bg-[rgb(var(--color-warning-rgb)/0.15)] p-2 text-warning">
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-1 text-xs text-(--text-muted)">
              ...
              <p className="font-mono text-[11px] text-(--text-muted)/90">
